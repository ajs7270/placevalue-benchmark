...
from core import PoT, PoT_original, CoT_original, CP_rendezvous, digit2alph, CP_rendezvous_d2a, _compare_single_token, \
    _compare_permutation, digit2alph_CoT, d2e_num_eng_matching, PoT_num_eng_matching
...
def prompting_test():
    try:
        outputs = []
        test_name = "PoT_num_eng_matching"
        #cot_filepath = "results/result_cot_original.json"
        for i, problem in enumerate(svamp):
            sleep(1)
            pot_cache, pot_output = PoT_num_eng_matching(llm=llm, problem=problem)
